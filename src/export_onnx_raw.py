...
medsam_lite_model.load_state_dict(torch.load("models_raw/3D.pth", map_location='cpu'))
medsam_lite_model.prompt_encoder.input_image_size=(256,256)
medsam_lite_model.to("cpu")
medsam_lite_model.eval()
os.makedirs("onnxmodels_raw", exist_ok=True)
np.save("onnxmodels_raw/positional_encoding.npy", medsam_lite_model.prompt_encoder.get_dense_pe().numpy()) #


models=glob.glob("models_raw/*")
...
for m in models:
    try:
        ...
        torch.onnx.export(medsam_lite_model.image_encoder,image, "onnxmodels_raw/"+model_name+"_image_encoder.onnx", input_names = ['image'], output_names = ['image_embedding'], dynamic_axes=dynamic_axes_img)
        torch.onnx.export(medsam_lite_model.prompt_encoder,(None, torch.tensor([[[[100.0,200,100,200]]]]), None), "onnxmodels_raw/"+model_name+"_prompt_encoder.onnx", input_names = ['boxes', 'points', 'masks'], output_names = ['sparse_embeddings', 'dense_embeddings'], dynamic_axes=dynamic_axes_prompt) # wrong input_names order, because it only keeps the first input name
        torch.onnx.export(medsam_lite_model.mask_decoder,mdin, "onnxmodels_raw/"+model_name+"_mask_decoder.onnx", input_names = list(mdin.keys()), output_names = ['low_res_masks', 'iou_predictions'])#, dynamic_axes=dynamic_axes_mdin)
        
        ie = ort.InferenceSession("onnxmodels_raw/"+model_name+"_image_encoder.onnx")
        pe = ort.InferenceSession("onnxmodels_raw/"+model_name+"_prompt_encoder.onnx")
        md = ort.InferenceSession("onnxmodels_raw/"+model_name+"_mask_decoder.onnx")
        positional_encoding = np.load("onnxmodels_raw/positional_encoding.npy")
